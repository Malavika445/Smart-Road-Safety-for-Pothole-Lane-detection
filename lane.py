...
#image_path = './output/original.png'
...
def abs_sobel_thresh(image, orient='x', sobel_kernel=3, thresh=(0, 255)):
    ...
    sobel = cv2.Sobel(gray, cv2.CV_64F, int(isX), int(not isX))
...
def apply_thresholds(image, ksize=3):
    ...
    print(combined.shape,mag_binary.shape,dir_binary.shape)
...
def process_image(image,imgpoints, objpoints):
    global used_warped
    global used_ret
    #objpoints, imgpoints=collect_callibration_points()
    #Undistort image
    image, mtx, dist_coefficients = cal_undistort(image, objpoints, imgpoints)
